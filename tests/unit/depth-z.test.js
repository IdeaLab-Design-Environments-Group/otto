...
test('every shape has depth=3, z=0 by default, both bindable', () => {
...
test('AQUI: depth: and z: flow through as ordinary shape params', () => {
    ...
    const result = runner.run('shape circle c1 { radius: 30 depth: 6 z: 10 }');
    assert(result.success, result.error);
    const shape = scene.shapeStore.getAll()[0];
    assertEqual(shape.depth, 6);
    assertEqual(shape.z, 10);
});
